...
let _show_official_rates = false;
...
frappe.ui.form.on("Costing Request", {
	onload(frm) {
		frappe.db.get_single_value("Costing Configuration", "production_days").then(v => {
			_config_production_days = v;
		});
		frappe.db.get_single_value("Costing Configuration", "supplier_financing_rate_pct").then(v => {
			_config_financing_rate = v;
		});
	},
...
function _render_action_bar(frm) {
	...
	frm.add_custom_button(
		_show_official_rates ? __("Hide Official Rates") : __("Show Official Rates"),
		() => {
			_show_official_rates = !_show_official_rates;
			_toggle_official_rates(frm, _show_official_rates);
			_render_action_bar(frm);
		}
	);
...
function _toggle_official_rates(frm, show) {
	// fetched_rate / fetched_supplier_credit_days are always visible (read-only) in edit dialog
	["fetched_charge_per_kg", "fetched_freight_per_unit", "fetched_includes_outward_freight"].forEach(f => {
		try { frm.fields_dict.processing_lines.grid.toggle_display(f, show); } catch (e) {}
	});
}
...
function _render_combination_card(frm, combo) {
	...
	const ml_rows = (combo.material_lines || []).map(ml => {
		const overridden = (Math.round((ml.working_rate || 0) * 100) / 100) !==
			(Math.round((ml.fetched_rate || 0) * 100) / 100);
		return `<tr>
			<td>${ml.item_name || ml.item} ${overridden ? "⚠" : "✓"}</td>
			<td>${(ml.qty_per_kg_output || 0).toFixed(4)} × ₹${(ml.working_rate || 0).toFixed(2)}</td>
			<td class="text-right">₹${(ml.amount_per_kg || 0).toFixed(2)}/kg</td>
		</tr>
		<tr class="text-muted small"><td colspan="3">&nbsp;&nbsp;
			${ml.supplier || "—"} | ${ml.working_supplier_credit_days || 0}d credit | ${ml.net_financed_days || 0}d financed
		</td></tr>`;
	}).join("");
...
function _render_cost_breakdown(frm, data) {
	...
	const rm_rows = (l.material_lines || []).map(ml => {
		const overridden = ml.is_overridden;
		const override_note = overridden ? ` ⚠ <em>(official ₹${(ml.fetched_rate || 0).toFixed(2)})</em>` : "";
		return `<div class="ml-3 mb-2">
			<strong>${ml.item_name || ml.item}</strong>${override_note}<br>
			<span class="text-muted">
				${(ml.qty_per_kg_output || 0).toFixed(4)} kg × ₹${(ml.working_rate || 0).toFixed(2)}
				= <strong>₹${(ml.amount_per_kg || 0).toFixed(2)}/kg</strong><br>
				${ml.supplier || "—"} | ${ml.working_supplier_credit_days || 0}d credit | ${ml.net_financed_days || 0}d financed
				| Financing: ₹${(ml.financing_cost_per_kg || 0).toFixed(2)}/kg
			</span>
		</div>`;
	}).join("");
	...
	$wrapper.html(`
		<div class="p-3 border rounded">
			...
			${rm_rows}
			<div class="font-weight-bold mt-2">FINANCING COST (${l.supplier_financing_rate_pct}% pa, ${l.production_days} days)</div>
